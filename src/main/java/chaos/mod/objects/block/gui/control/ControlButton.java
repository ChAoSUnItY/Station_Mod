...
public class ControlButton extends Control {
	...
	public ControlButton(GuiScreen gui) {
		super(gui);
	}

	public ControlButton(GuiScreen gui, int x, int y, int w, int h, String tx) {
		super(gui);
		this.locationX = x;
		this.locationY = y;
		this.width = w;
	...
	public void draw(int mouseX, int mouseY) {
		...
		drawRect(this.locationX + 1, this.locationY + this.height - 1, this.locationX + this.width,
				this.locationY + this.height, bottomColor);
		drawRect(this.locationX + this.width - 1, this.locationY, this.locationX + this.width,
				this.locationY + this.height, bottomColor);
		getFont().drawStringWithShadow(this.text,
				this.width / 2 - getFont().getStringWidth(this.text) / 2 + this.locationX,
				this.height / 2 - 4 + this.locationY, fontColor);
	}
